...
2. Add a domain, e.g. `rt.example.com`.
...
- `https://rt.example.com/api` (POST with `X-API-Key` header) → JSON response (server-to-server admin API)
- `wss://rt.example.com/connection/websocket` → WebSocket handshake (browser client transport)

> ℹ️ **About `/health`**: a public `/health` endpoint is **not** part of this deployment's contract. Coolify has no Docker healthcheck wired in (see §2.1), and we deliberately do not advertise `/health` publicly. If `curl https://rt.example.com/health` returns `404` or `503`, that is **not an error** — verify health via the steps in §5 instead.
...
```
WIDGET_ASSET_BASE_URL=https://app.example.com   # frontend public URL
```
...
After Centrifugo is running on `https://rt.example.com`, log into the platform as a super admin and go to **Super Admin → Providers → Realtime**. Set:
...
| **WebSocket URL** (`ws_url`) | `wss://rt.example.com/connection/websocket` |
| **HTTP API URL** (`api_url`) | `https://rt.example.com/api` |
...
> ⚠️ **Do not test `https://rt.example.com/health`.** It is intentionally not exposed publicly (see §2.2). A `404`/`503` there does not mean Centrifugo is down — use the tests below instead.
...
```bash
curl -i -X POST https://rt.example.com/api \
...
```bash
curl -i https://rt.example.com/connection/websocket
...
```bash
websocat -v wss://rt.example.com/connection/websocket
...
2. In DevTools → Network, look for `POST /api/realtime/connect`. Response should contain:
   ```json
   { "vendor": "centrifugo", "ws_url": "wss://rt.example.com/connection/websocket", "token": "...", ... }
   ```
3. Then a WebSocket connection to `wss://rt.example.com/connection/websocket` should open with status `101 Switching Protocols` and stay open.
...
- **`curl https://rt.example.com/health` returns `404`/`503`** → expected, not a bug. The public domain only routes the two endpoints we actually need (`/api` and `/connection/websocket`). Verify health using §5.1, §5.2, and §5.4 instead.
