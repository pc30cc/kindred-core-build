-- 019 — Account security: server-only RPCs to list/revoke auth.sessions.
-- PostgREST cannot expose the `auth` schema, so the API server reads sessions
-- through these SECURITY DEFINER helpers (service_role only).
