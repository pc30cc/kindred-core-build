...
-- password. Fixed the same way as password reset (030): the write and the
...
BEGIN
  IF to_regprocedure('public.change_password_and_revoke_sessions(uuid,text,uuid)') IS NULL THEN
    RAISE EXCEPTION '031: change_password_and_revoke_sessions was not created';
  END IF;
  IF has_function_privilege('anon', 'public.change_password_and_revoke_sessions(uuid,text,uuid)', 'EXECUTE') THEN
    RAISE EXCEPTION '031: anon can execute change_password_and_revoke_sessions';
  END IF;
  IF has_function_privilege('authenticated', 'public.change_password_and_revoke_sessions(uuid,text,uuid)', 'EXECUTE') THEN
    RAISE EXCEPTION '031: authenticated can execute change_password_and_revoke_sessions';
  END IF;
  RAISE NOTICE '031: change_password_and_revoke_sessions created, locked to service_role';
END
$verify$;
