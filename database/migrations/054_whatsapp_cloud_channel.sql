-- 054_whatsapp_cloud_channel.sql
--
-- Adds the Meta WhatsApp Cloud channel alongside Telegram and Bale.
--
-- WhatsApp speaks a different wire protocol (Graph API), but that difference
-- is confined to the Channels Worker: inbound payloads are translated into
-- the shared bot envelope at Core's ingest boundary, so the database side is
-- identical to any other bot channel — the outbox trigger just needs to know
-- the provider exists.
--
-- Idempotent: safe to re-run.

-- ── 1. Outbound intent uniqueness covers WhatsApp job types ───────────
...
-- ── 2. Outbox trigger recognises the WhatsApp channel ─────────────────
...
BEGIN
  ...
  IF c_channel NOT IN ('telegram', 'bale', 'whatsapp') THEN
    RETURN NEW;
  END IF;
...
VALUES ('whatsapp', true, true, true, true, 16, 'public')
