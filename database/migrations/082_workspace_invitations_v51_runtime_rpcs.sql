-- 082 — Workspace Invitations v5.1: runtime RPCs (preview, OTP/proof,
-- login-context, outbox worker lifecycle) — self-host chain.
--
-- Numbers 080/081 are deliberately reserved for the cutover-only fence and
-- contract scripts under database/cutover/. This file is part of the normal
-- migration chain.
--
-- Rules honoured here (v5.1 §5, §6, §9, §10, §17):
--   * service_role-only, SECURITY DEFINER, SET search_path = public, pg_temp.
--   * Canonical lock order: workspace -> invitation -> token/OTP/proof/context
--     -> job.
--   * Express never passes raw tokens/OTPs/proofs: only sha256/HMAC digests.
--   * Uniform INVITATION_NOT_FOUND for every public-surface failure.
--   * No dynamic SQL in business logic.

-- =========================================================================
-- 0. Masked preview (read-only, no locks, no mutation)
-- =========================================================================
...
-- =========================================================================
-- 1. OTP request (manual handoff only)
-- =========================================================================
-- Express generates the code and passes only its peppered HMAC digest.
...
BEGIN
  ...
  -- A new OTP invalidates every previous unconsumed one.
...
BEGIN
  ...
  -- Only ONE live proof per invitation.
...
-- =========================================================================
-- 3. Login round-trip context (blocker 3)
-- =========================================================================
...
-- Resolves a login context to the STORED TOKEN HASH (never a raw token) so
-- the acceptance RPC contract stays unchanged. Consumes the context.
...
-- =========================================================================
-- 4. Outbox worker lifecycle
-- =========================================================================
-- Persists the derived email-claim token hash for a claimed job and returns
-- the (secret-free) recipient payload. Commits BEFORE the provider call.
...
BEGIN
  ...
  IF _job.channel = 'email' THEN
    ...
    -- Previous email tokens of this invitation die; this job's token becomes
    -- the only live email claim token.
...
-- Read-only preflight immediately before provider submission.
...
BEGIN
  ...
  -- A worker that lost its lease can never overwrite newer state.
