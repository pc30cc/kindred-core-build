-- =========================================================================
-- WORKSPACE INVITATIONS v5.1 §10 — idempotency safe-result projection fix
-- =========================================================================
-- Forward-only follow-up to 087. The committed ledger projection dropped the
-- invitation identity for every primitive that returns the safe invitation
-- itself (create/edit/revoke/archive) or wraps it (resend/rotate), so a replay
-- after a lost response could not tell the caller WHICH invitation had already
-- been committed. The allow-list is unchanged: still no token, OTP, proof,
-- password, session or raw contact detail may ever reach the ledger.
-- =========================================================================
