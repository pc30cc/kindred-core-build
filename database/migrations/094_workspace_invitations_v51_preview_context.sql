-- =========================================================================
-- WORKSPACE INVITATIONS v5.1 — Section E preview context
--
-- The invited-user screen (/invite) must show WHO invited the person and,
-- for customer-facing invitations, WHICH departments they are joining.
-- Both preview projections stay read-only, secret-free and identical in the
-- self-host and hosted chains:
--   * public.wi_preview_invitation(text, text)   -> jsonb
--   * public.wi_preview_login_context(text)      -> jsonb
--
-- Added keys (both functions):
--   inviter_name      text|null   — display name of the inviter, never email
--   department_names  text[]      — customer-facing routing departments only
--
-- No token hash, handle, raw email, raw phone or OTP digest is exposed.
-- =========================================================================
