-- Iran billing redesign — server-authoritative payment intents.
--
-- Checkout amount must NEVER be trusted from the client: the intent row is
-- created server-side (Express) from the priced plan (subscription renewal/
-- upgrade) or from a validated AI-credit top-up amount BEFORE the provider
-- redirect happens, and verify-callback re-reads the SAME row to drive the
-- gateway verify call and the resulting subscription period / credit grant.
...
COMMENT ON TABLE public.billing_payment_intents IS
  'Server-created, tamper-proof record of what a checkout is FOR (plan/interval or AI credit top-up) and the exact amount_irr charged, created before the provider redirect. verify-callback re-reads this row instead of trusting client-supplied amount/plan.';
