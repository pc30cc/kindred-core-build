...
--
-- Problems fixed:
--   1. The intent was flipped to `succeeded` BEFORE the financial side effect
--      (subscription period / AI credit grant) was applied. A crash in between
--      left the customer charged with nothing applied, and every retry was
--      treated as a replay. A `processing` state makes the window recoverable.
--   2. AI credit top-ups were only recorded in `billing_events`
--      (operational/audit), so they never appeared in the customer's payment
--      history. `billing_payments` becomes the single customer-facing
--      financial transaction table.
--   3. `billing_payments` had no replay-safe uniqueness and no snapshot of
--      WHAT was bought, so transaction history had to guess from the plan the
--      workspace happens to be on today.
--   4. The subscription did not persist which interval (monthly/yearly) was
--      actually purchased, so the UI showed the monthly price to yearly
--      customers.
...
-- IRR amounts exceed int4 for large yearly plans / top-ups.
...
-- Replay safety: one payment row per intent, and one per provider reference.
