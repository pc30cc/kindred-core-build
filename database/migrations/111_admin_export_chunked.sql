-- 111_admin_export_chunked.sql
-- The single-shot admin_export_database() serialises every public table into
-- one jsonb value and hits Postgres' statement_timeout on real installs.
-- Backups are now streamed table-by-table (and page-by-page) from the Express
-- backend using the two helpers below. The monolithic function is kept for
-- compatibility but gets a longer timeout.
