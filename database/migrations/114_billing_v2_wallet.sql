...
--
-- INVARIANT: wallet money, purchased AI credit and plan AI allowance are three
-- separate financial/entitlement domains and must NEVER share a balance.
--
--   * Wallet  = real customer money (IRR), spendable on any invoice.
--   * Purchased AI credit = an AI entitlement lot (073 ledger). It can never
--     pay a subscription invoice.
--   * Plan AI allowance   = period-bound AI entitlement (113 periods).
--
-- `billing_wallet_accounts.available_balance_irr` is a DENORMALIZED CACHE.
-- The source of truth is the append-only `billing_wallet_ledger`; the cache is
-- only ever written by the SQL functions in 115, inside the same transaction
-- and under a row lock.
-- ============================================================
...
CREATE TABLE IF NOT EXISTS public.billing_wallet_ledger (
  ...
  -- Signed: credits are positive, debits negative. The sum of this column for
  -- a workspace IS the balance; the account row only caches it.
...
-- Durable command idempotency: a replayed deposit/auto-pay writes nothing new.
...
-- Close the loop on the intent → deposit relation declared in 113.
...
-- The abstraction that makes partial payment possible without any UI change:
-- an invoice is settled by one or more allocations, each backed either by a
-- gateway payment or by a wallet ledger entry.
...
-- Replay safety: the same settlement command allocates once.
...
-- A single gateway payment can never be allocated to the same invoice twice.
