...
--
-- Migrations 113–116 are FROZEN (validated on real PostgreSQL in Phase A).
-- Every Phase B database guard lives here, additively.
--
-- What this migration establishes:
--
--   1. A canonical, server-authoritative, PER-WORKSPACE rollout state
--      (legacy → shadow → v2_cutover_pending → v2_active). No global boolean
--      exists, so a mixed V1/V2/shadow population is the normal case.
--   2. `billing_engine_version` stamped IMMUTABLY on every financial object,
--      so a late callback is routed by the object it belongs to and never by
--      the workspace's current state.
--   3. A DB-LEVEL guard: once a workspace is `v2_active`, the legacy
--      "payment → subscription" mutation is rejected by the database itself,
--      even if a forgotten route, a bug or a future regression calls it.
--   4. A DB-LEVEL guard: once V2 owns the AI allowance, no legacy calendar
--      grant can create another plan-allowance lot.
--   5. A fail-closed readiness evaluator and an ATOMIC activation RPC that
--      locks, re-checks, drains unbound legacy intents, creates the first
--      period when needed, transitions state and audits — in one transaction.
--
-- Monotonicity: after `v2_active`, returning to `legacy` is impossible through
-- the normal API. A rollback would risk a double period / double allowance, so
-- it is a deliberate break-glass operation (`p_break_glass`), always audited.
-- ============================================================
...
-- The stamp answers "which engine created this object?" WITHOUT consulting the
-- workspace's current state, which is exactly what a late callback needs.
...
-- A period backfilled from the legacy contract describes V1 service.
...
--
-- Once a workspace is `v2_active`, the ONLY legitimate writer of the
-- money-authority columns is V2 period activation, which always leaves the
-- subscription row consistent with an ACTIVE period row it points at. The
-- legacy `billing_apply_subscription_payment` RPC knows nothing about periods,
-- so it can never satisfy that invariant — and is rejected by the database.
...
-- V2 grants are period-bound (`billing_cycle_id = 'period:<uuid>'`). A legacy
-- calendar grant (`YYYY-MM`) for a workspace whose allowance authority has
-- already moved to V2 would fund the same service twice.
...
-- ─── 9. Atomic activation ─────────────────────────────────────────────────
--
-- lock → re-evaluate readiness → drain unbound legacy intents → ensure wallet
-- → ensure a service period → transition → audit. All in ONE transaction, so a
-- concurrent legacy checkout or a second admin click cannot interleave.
