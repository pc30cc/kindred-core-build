-- ============================================================================
-- BILLING ENGINE V2 — PHASE C
-- Renewal Invoice Scheduler + Wallet Auto-Pay + Period Activation Engine.
--
-- Migrations 113–117 are FROZEN. Everything new lives here.
--
-- Three workers, one shared durable job substrate:
--   A. renewal_invoice   — subscription → next renewal invoice (lead time)
--   B. wallet_autopay    — due invoice + sufficient wallet → invoice paid
--   C. period_activation — paid/scheduled service period → active at start
--
-- None of them depends on an in-memory timer: every unit of work is a row with
-- status / attempt_count / lease_until / next_attempt_at / last_error, so a
-- crash or a restart loses nothing and a second instance cannot double-run it.
-- ============================================================================

-- ─── 1. Billing policy (single source for lead time and auto-pay default) ──
...
-- Per-workspace override (NULL column = inherit the platform policy).
...
/**
 * Resolves the effective policy for a workspace.
 * Order: workspace override → wallet account preference → platform default.
 */
...
-- ─── 2. Durable job substrate shared by the three workers ──────────────────
...
/** Claims a bounded batch with SKIP LOCKED — multi-instance safe. */
...
/**
 * A transient skip is NOT a success and NOT a failure: the work item goes back
 * to pending without burning a retry, so the next run re-evaluates it. This is
 * what lets an invoice blocked by a live gateway collection be collected later.
 */
...
-- Worker health / readiness surface.
...
-- ─── 3. Allowance application ledger (exactly once, recoverable) ───────────
...
DECLARE
  ...
  v_last   TEXT;
BEGIN
  -- Enqueue every eligible subscription (catch-up: <= now, not = now).
  FOR r IN
...
/**
 * Activation, Phase C edition. Additions over the 115 version:
 *   • an invoice-backed period may only activate when its invoice is PAID
 *   • the AI allowance grant is recorded in a durable ledger row, so a crash
 *     between activation and grant is recoverable instead of silent
 *   • replay is still exactly once, and the grant command key stays
 *     plan_allowance:<period_id>
 */
...
BEGIN
  ...
  -- The subscription row is the pointer we are about to move: lock it too.
  ...
  -- Durable allowance work item, then the grant itself.
...
/** Grants the period's plan allowance — exactly once, from the snapshot. */
...
-- ─── 9. Scheduler health / metrics (read-only) ─────────────────────────────
