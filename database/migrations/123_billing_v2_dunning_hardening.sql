-- ============================================================================
-- BILLING ENGINE V2 — 123: DUNNING HARDENING (closes Phase E).
--
-- Migrations 113–122 are FROZEN. Everything here is additive or a redefinition
-- of a 122 function body; no 122 object is dropped.
--
-- What this migration adds, and why each one matters:
--
--   1. A FROZEN dunning snapshot on the invoice at the moment it becomes
--      collectible. Grace length and the fallback plan are the contract the
--      customer received; a later Super Admin policy change must never shorten
--      the grace of an invoice that is already in flight.
--   2. Automatic arming: opening an invoice schedules its reminders. Without
--      this the reminder ladder depends on someone remembering to call it.
--   3. `billing_v2_process_due_invoice` reads grace from the frozen snapshot,
--      audits `invoice_due`, and reports `auto_pay_disabled` as a customer
--      choice rather than a system failure.
--   4. `billing_v2_apply_free_fallback` RAISES `fallback_plan_not_configured`
--      instead of guessing a free plan. Cutting a paying customer down to a
--      plan nobody configured is worse than staying past_due.
--   5. A late payment landing after fallback is AUDITED, never silently
--      reviving the old plan.
--   6. The renewal scheduler refuses to stack a second renewal invoice on a
--      workspace that already has an unpaid one.
-- ============================================================================
...
BEGIN
  ...
  IF v_snap IS NULL OR jsonb_typeof(v_snap) <> 'object' THEN
    -- Never invented retroactively as "whatever policy says today" for an
    -- invoice that predates this migration: the live policy is the only
    -- honest answer, and it is returned explicitly marked as unfrozen.
...
/** Freezes the snapshot the first time an invoice becomes collectible. */
...
-- ─── 2. Arming: opening an invoice schedules its reminder ladder ───────────
...
BEGIN
  ...
  -- (3) Wallet auto-pay — full payment or nothing.
  ...
  IF v_sub.id IS NOT NULL AND v_sub.status IN ('active', 'past_due') THEN
    -- Grace comes from the FROZEN snapshot: a policy change today cannot
    -- shorten the grace of an invoice the customer already received.
    v_grace := COALESCE(
...
BEGIN
  ...
  IF v_plan_id IS NULL THEN
    -- Guessing a plan here would silently reshape a customer's entitlements.
    -- Staying past_due is the safe failure: service is NOT cut off.
...
-- ─── 5. A late payment after fallback is audited, never a silent revival ───
...
-- ─── 6. No stacking of renewal invoices on an unpaid workspace ─────────────
    ...
    SELECT s.workspace_id, s.id AS sub_id, COALESCE(s.next_invoice_at, s.current_period_end) AS anchor
     ...
     WHERE s.status IN ('active', 'past_due')
       ...
       AND COALESCE(s.next_invoice_at, s.current_period_end)
           ...
           <= now()
       -- Never stack a renewal on top of money the customer already owes.
       AND NOT EXISTS (
...
/**
 * `profiles.preferred_locale` is an optional column in this product (some
 * deployments carry it, some do not). Referencing it statically made every
 * notification enqueue fail on deployments without it — a dunning engine that
 * cannot address the customer is worse than one with no locale preference.
 */
...
BEGIN
  ...
  -- Fall back to the profile phone when the deployment carries one.
...
/**
 * A voided invoice must not leave an "issued" or "payment received" message
 * queued behind it. Cancellation covers the invoice's whole message set, not
 * just the reminder subset.
 */
...
/**
 * Recovery must not depend on WHICH code path settled the invoice: wallet,
 * gateway callback or an operator write all mean the same thing to the
 * customer — the debt is gone, so the service comes back.
 */
...
/**
 * A skip is a terminal, non-error outcome: no recipient on the channel, or a
 * policy that suppressed the message. It must never be retried and must never
 * be counted as a delivery failure.
 */
