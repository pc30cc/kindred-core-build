...
--
-- Two additive things, nothing destructive, nothing retroactive:
--
--   1. `billing_v2_policy.new_workspace_default_state` (+ region). A workspace
--      created AFTER this migration is seeded with that rollout state, so V2 is
--      the default road for new Iran workspaces. EXISTING workspaces are never
--      touched: legacy stays legacy until a canonical cutover moves it.
--
--      Seeding a brand-new workspace is financially inert: it has no
--      subscription, no period, no payment intent and no invoice, so there is
--      nothing V1 could still own. The free period is materialised later by
--      `billing_v2_ensure_free_period` in the invoice scheduler, exactly as for
--      a workspace activated through `billing_v2_activate`.
--
--   2. `billing_v2_validate_policy()` — the single, canonical answer to
--      "is the platform billing policy safe to run V2 on?". The production
--      cutover runbook calls THIS; it never re-implements the rules.
-- ============================================================================

-- ─── 1. Default rollout state for newly created workspaces ────────────────
...
-- ─── 2. Canonical mandatory-policy validator ──────────────────────────────
--
-- Returns { ok, errors[], warnings[], policy{} }. `ok = false` MUST stop a
-- production cutover. The fallback plan is the critical one: when grace expires
-- unpaid, a workspace lands on it, so it must be a real, active, free plan.
...
BEGIN
  ...
  IF v_pol.fallback_plan_id IS NULL THEN
    -- NULL is legal (first free plan is used) but only if one actually exists.
    IF v_free = 0 THEN
