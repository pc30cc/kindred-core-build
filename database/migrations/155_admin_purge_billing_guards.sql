-- Admin purge: let admin_delete_user / admin_delete_workspace remove a
-- workspace whose billing history is protected by append-only guards.
--
-- Deleting a workspace touches frozen financial rows indirectly (for example
-- deleting an invoice nulls billing_subscription_periods.invoice_id via
-- ON DELETE SET NULL, which the freeze trigger rejects with
-- "subscription_period_immutable"). Money guards must stay in force for the
-- application; they are relaxed ONLY inside the platform-admin purge RPCs,
-- which run as SECURITY DEFINER and set a transaction-local flag.
...
BEGIN
  IF coalesce(current_setting('app.billing_purge', true), '') <> 'on' THEN
    RETURN false;
  END IF;
  -- Only the database owner / service role may ever be inside a purge.
...
-- Inject the purge escape hatch at the top of every billing/AI guard trigger.
DO $$
DECLARE
  v_name text;
  v_def  text;
...
-- Turn the flag on inside the two platform-admin purge RPCs.
DO $$
DECLARE
  v_name text;
  v_def  text;
