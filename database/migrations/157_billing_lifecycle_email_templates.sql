-- ============================================================
-- 157 — BRANDED EMAIL TEMPLATES FOR THE LIFECYCLE NOTIFICATIONS
--
-- Migration 156 added the lifecycle notification types, but no rows existed
-- in `public.email_templates` for the new slugs. The dispatcher then fell
-- back to its plain-text English rendering: unbranded, untranslated mail.
--
-- This migration seeds the missing slugs in fa/en/tr using the same branded
-- shell as the existing billing templates ({brand}, {year}, {action_url}),
-- and makes the recipient locale fall back to the platform default locale
-- instead of a hardcoded 'fa'/'en'.
...
CREATE OR REPLACE FUNCTION public._seed_billing_email_template(
  ...
  p_title    TEXT,
  p_intro    TEXT,
  p_rows     TEXT,          -- pre-rendered <tr> detail rows (may be '')
  p_cta      TEXT,
  p_text     TEXT
) RETURNS VOID
LANGUAGE plpgsql
...
BEGIN
  v_html :=
    ...
    CASE WHEN COALESCE(p_rows, '') = '' THEN '' ELSE
      '<tr><td style="padding:0 40px;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" ' ||
      'style="background-color:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:8px 16px;text-align:' || v_align || ';">' ||
      p_rows || '</table></td></tr>'
    END ||
...
-- Small helper for a label/value row honouring direction.
...
BEGIN
  ...
  PERFORM public._seed_billing_email_template('wallet_deposit_received','fa',
    ...
    public._seed_billing_email_row('fa','مبلغ','{amount}'),
    'مشاهده کیف پول',
  ...
  PERFORM public._seed_billing_email_template('wallet_deposit_received','en',
    ...
    public._seed_billing_email_row('en','Amount','{amount}'),
    'View wallet',
  ...
  PERFORM public._seed_billing_email_template('wallet_deposit_received','tr',
    ...
    public._seed_billing_email_row('tr','Tutar','{amount}'),
    'Cüzdanı görüntüle',
...
-- ── Recipient locale follows the platform default ─────────────────────
-- Previously hardcoded to 'fa'; a profile row whose preferred_locale was
-- never chosen by the user carries the column default 'en', which made
-- billing mail English on a Persian platform. The account choice still
-- wins when it exists; otherwise the platform default locale decides.
