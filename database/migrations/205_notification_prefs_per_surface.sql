-- ============================================================
-- 205 — ONE SET OF NOTIFICATION PREFERENCES PER OPERATOR, PER SURFACE
--
...
BEGIN
  ...
  IF NOT has_platform THEN
    RAISE EXCEPTION '205: user_notification_prefs.platform is missing';
  END IF;
  IF NOT unique_covers_platform THEN
    RAISE EXCEPTION '205: the unique index does not cover platform';
  END IF;

  RAISE NOTICE '205 ok: preferences are per operator, per surface';
END
$verify$;
