-- ============================================================
-- 206 — THE NOTIFICATION EMAILS THAT WERE ONLY EVER SWITCHES
--
...
BEGIN
  ...
  IF settings_rows <> 1 THEN
    RAISE EXCEPTION '206: notification_email_settings must hold exactly one row, found %', settings_rows;
  END IF;
  IF seeded < 15 THEN
    RAISE EXCEPTION '206: expected 5 templates in 3 locales, found %', seeded;
  END IF;
  IF NOT has_prefs THEN
    RAISE EXCEPTION '206: user_email_notification_prefs is missing';
  END IF;
  IF NOT has_dedupe THEN
    RAISE EXCEPTION '206: the queue has no dedupe index, so a digest can go twice';
  END IF;

  RAISE NOTICE '206 ok: notification emails have a platform switch, a queue, preferences and copy';
END
$verify$;
