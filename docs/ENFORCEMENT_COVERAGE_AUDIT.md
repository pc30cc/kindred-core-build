...
| `server/routes/callQueue.ts`, `callAvailability.ts` | `requireModule('call_center')` + `requireFeature('call_queue')` | New plan keys exist as of the call-surface plan-model phase; gating deferred until plans are backfilled and effective state composes correctly with `loadEffectiveCallChannels`. See `CALL_SURFACES_PLAN_MODEL.md`. |
| `server/routes/callbacks.ts`, `widgetCallbacks.ts` | `requireFeature('call_callbacks')` | New plan key; deferred for the same reason as `call_queue`. |
| recording endpoints | `requireFeature('call_recording')` | Plan key now exists; no recording HTTP surface is currently in scope to gate. |
