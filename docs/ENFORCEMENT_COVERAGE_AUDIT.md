...
| `server/routes/calls.ts` POST `/:id/invite` | `eff.voice_enabled` / `eff.video_enabled` | DEFERRED — acts on an already-existing `call_sessions` row and may be re-issued mid-call to add additional participants; gating would strand in-flight calls. Held until a participant-class drain decision is made. |
...
| `server/routes/callQueue.ts` (offer/accept), `widget` `call-queue/enqueue` | composed call entitlements | DEFERRED — operate on already-existing queue entries OR are public/widget surfaces with undefined denial UX. Need per-surface drain decision before gating. |
