...
| `server/routes/callInvitations.ts` POST `/` | composed via `loadEffectiveCallEntitlements` | **GATED (Phase: Call Route Enforcement Rollout — Strict Minimal Pass).** Channel-scoped: audio requires `voice_video ∧ voice ∧ runtime.voice_enabled`, video requires `voice_video ∧ video ∧ runtime.video_enabled`. Denial returns `403 plan_forbidden` and never invokes `createInvitation`. Cancel/get/list ungated to preserve in-flight invitation visibility. |
| `server/routes/callInvitations.ts` cancel/get/list, `callQueue.ts`, `callAvailability.ts`, `workspaceCalls.ts`, `widgetCallInvitations.ts`, `widgetCallbacks.ts` | composer exists, not yet wired | Remaining call routes still deferred: in-flight queue/offer/cancel semantics need a "deny on create, allow on cleanup" policy before broader gating. See `CALL_ENTITLEMENT_COMPOSITION.md`. |
