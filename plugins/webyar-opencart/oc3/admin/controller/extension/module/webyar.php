 ...
 * `extension/module/webyar` (granted to the installing group by the
...
class ControllerExtensionModuleWebyar extends Controller {
		...
		if (!$json) {
			...
			if ($app === '') {
				...
				try {
					...
					$callback = $store['url'] . 'index.php?route=extension/module/webyar/callback';
	...
	public function install() {
		...
		if (!self::supported() || !$this->user->hasPermission('modify', 'extension/extension/module')) {
			return;
		}
