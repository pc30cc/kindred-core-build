 ...
 *   • Send now              → calls the operator's normal send flow, applying
 *                             the send mode selected in the composer toolbar.
...
interface Props {
  ...
  /** Send mode selected in the composer toolbar (shared with the Send button). */
...
export function AiSuggestionCard({
  ...
  const actionMeta: Record<PostSendAction, { label: string; short: string; icon: typeof Send }> = {
    none: {
  ...
  const handleSendNow = async () => {
    ...
    setSending(true);
    try {
      const ok = await Promise.resolve(onSendNow(suggestion.suggested_reply, sendAction));
      if (ok) {
