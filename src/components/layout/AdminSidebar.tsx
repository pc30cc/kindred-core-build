...
export function AdminSidebar({ mobileOpen = false, onMobileClose }: { mobileOpen?: boolean; onMobileClose?: () => void }) {
    ...
    >
      <div className="flex h-16 items-center justify-between border-b border-border/70 px-3">
        {!collapsed && (
          <div className="flex min-w-0 items-center gap-3 px-1">
            ...
            <div className="min-w-0"><p className="truncate text-sm font-bold">{t('admin.nav.title')}</p><p className="truncate text-[11px] text-sidebar-muted-foreground">{t('admin.nav.platformManagement')}</p></div>
          </div>
        )}
        <button onClick={() => setCollapsed(!collapsed)} className="hidden rounded-lg p-2 text-sidebar-muted-foreground transition-colors hover:bg-sidebar-accent hover:text-sidebar-accent-foreground lg:block" aria-label={t('admin.nav.collapseMenu')}>
          <CollapseIcon className="h-4 w-4" />
        </button>
        <button onClick={onMobileClose} className="rounded-lg p-2 text-sidebar-muted-foreground hover:bg-sidebar-accent lg:hidden" aria-label={t('admin.nav.closeMenu')}><X className="h-5 w-5" /></button>
      </div>
      ...
      <nav className="admin-scrollbar flex-1 overflow-y-auto px-2 py-3">
        {adminNav.map(item => {
          ...
          {showGroup && <p className="mb-1 mt-4 px-3 text-[10px] font-semibold uppercase tracking-[0.16em] text-sidebar-muted-foreground first:mt-1">{t(`admin.nav.groups.${item.group}` as TranslationKey)}</p>}
          <Link
            key={item.key}
            to={item.path}
            onClick={onMobileClose}
            title={collapsed ? t(`admin.nav.${item.key}` as TranslationKey) : undefined}
            className={cn(
          ...
          >
            ...
            {!collapsed && <span className="truncate">{t(`admin.nav.${item.key}` as TranslationKey)}</span>}
          </Link>
          </div>;
        })}
      </nav>
  ...
  return <>
    ...
    {mobileOpen && <button className="fixed inset-0 z-40 bg-black/55 backdrop-blur-sm lg:hidden" onClick={onMobileClose} aria-label={t('admin.nav.closeMenu')} />}
