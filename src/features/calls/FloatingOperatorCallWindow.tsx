...
export function FloatingOperatorCallWindow() {
  ...
  const doTransfer = async (agentId: string) => {
    ...
    try {
      ...
      toast({
        title: safeT('inbox.callSurface.transferSuccess', 'Call transferred'),
        description: safeT('inbox.callSurface.transferSuccessDesc', 'Routing to the selected operator…'),
      });
    } catch (e: any) {
      toast({
        title: safeT('inbox.callSurface.transferFailed', 'Transfer failed'),
          ...
          {!isRemoteEndedTerminal && surface.phase === 'connected' && callId && (
            <Popover open={transferOpen} onOpenChange={setTransferOpen}>
              <PopoverTrigger asChild>
                <Button size="sm" variant="outline" className="h-12 w-12 rounded-full border-call-stage-foreground/20 bg-card/75 p-0 text-foreground shadow-elevated backdrop-blur-xl hover:bg-card" aria-label={safeT('inbox.callSurface.transferCall', 'Transfer call')} title={safeT('inbox.callSurface.transferCall', 'Transfer call')}>
                  <ArrowRightLeft className="h-5 w-5" />
                </Button>
              </PopoverTrigger>
              <PopoverContent align="end" className="w-72 p-2">
                <div className="px-2 py-1 text-[11px] font-bold uppercase tracking-wide text-muted-foreground">{safeT('inbox.callSurface.transferTo', 'Transfer to')}</div>
                {availableTargets.length === 0 ? (
                  <div className="px-2 py-3 text-sm text-muted-foreground">{safeT('inbox.callSurface.noAvailableOperators', 'No operator is available right now')}</div>
                ) : (
                  <div className="flex max-h-64 flex-col gap-1 overflow-auto py-1">
                    {availableTargets.map((t) => (
                      ...
                      >
                        ...
                        <span className="text-[10px] font-bold uppercase text-success">{safeT('inbox.callSurface.operatorAvailable', 'Available')}</span>
                      </button>
                    ))}
                  </div>
                )}
