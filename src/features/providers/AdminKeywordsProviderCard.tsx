/**
 * ADMIN — SEO KEYWORD RESEARCH PROVIDER CARD
 *
 * Mirrors AdminBacklinksProviderCard.tsx's shape. Talks exclusively to
 * `/api/admin/providers/seo-keywords`, which returns a redacted view
 * (`hasCredentials`) and never the stored credential itself.
 */

import { useCallback, useEffect, useState } from 'react';
import { Search, CheckCircle2, XCircle, Loader2, ShieldCheck, Wallet } from 'lucide-react';
...
import {
  adminGetKeywordsProvider, adminSaveKeywordsProvider, adminDeleteKeywordsProvider, adminTestKeywordsProvider,
  type AdminKeywordsProviderInfo, type AdminKeywordsTestResult,
} from '@/lib/api';

export function AdminKeywordsProviderCard() {
  const { toast } = useToast();
  const [info, setInfo] = useState<AdminKeywordsProviderInfo | null>(null);
  const [loading, setLoading] = useState(true);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [saving, setSaving] = useState(false);
  const [testing, setTesting] = useState(false);
  const [testResult, setTestResult] = useState<AdminKeywordsTestResult | null>(null);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setInfo(await adminGetKeywordsProvider());
    } catch {
      setInfo(null);
    } finally {
      setLoading(false);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const next = await adminSaveKeywordsProvider({
      ...
      setPassword('');
      setDialogOpen(false);
      toast({ title: 'Keywords provider saved' });
    } catch (err) {
      toast({
        title: 'Could not save keywords provider',
  ...
  const handleTest = async () => {
    ...
    setTestResult(null);
    try {
      setTestResult(await adminTestKeywordsProvider());
    } catch (err) {
      setTestResult({
        success: false,
  ...
  const handleDisable = async () => {
    try {
      setInfo(await adminDeleteKeywordsProvider());
      setTestResult(null);
      toast({ title: 'Keywords provider removed' });
    } catch (err) {
      toast({
        title: 'Could not remove keywords provider',
        description: err instanceof Error ? err.message : 'Unknown error',
        variant: 'destructive',
      });
    }
  ...
  return (
    <Card className="bg-card border-border" data-testid="admin-keywords-provider-card">
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between gap-2">
          <CardTitle className="text-sm flex items-center gap-2 text-foreground">
            <Search className="h-4 w-4 text-primary" />
            SEO Keyword Research Provider
          </CardTitle>
          ...
          <>
            <div className="space-y-1.5 text-xs">
              ...
              <div className="flex items-center justify-between">
                ...
                {info?.hasCredentials ? (
                  <span className="flex items-center gap-1 text-emerald-500" data-testid="keywords-credential-saved">
            ...
            {testResult && (
              <div
                ...
                data-testid="keywords-test-result"
              >
                {testResult.success ? (
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-sm">
              <Search className="h-4 w-4 text-primary" /> Configure Keyword Research Provider
            </DialogTitle>
          ...
          <div className="space-y-3">
            <div className="space-y-1.5">
              <Label htmlFor="keywords-login" className="text-xs">Login</Label>
              <Input
                id="keywords-login"
                autoComplete="off"
                value={login}
            ...
            <div className="space-y-1.5">
              <Label htmlFor="keywords-password" className="text-xs">Password</Label>
              <Input
                id="keywords-password"
                type="password"
                autoComplete="off"
                value={password}
            ...
            <div className="flex items-center justify-between rounded-md border border-border px-3 py-2">
              <Label htmlFor="keywords-enabled" className="text-xs">Enabled</Label>
              <Switch id="keywords-enabled" checked={enabled} onCheckedChange={setEnabled} />
            </div>
          </div>
