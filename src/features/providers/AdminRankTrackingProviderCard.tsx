/**
 * ADMIN — SEO RANK TRACKING PROVIDER CARD
 *
 * Mirrors AdminBacklinksProviderCard.tsx's shape. Talks exclusively to
 * `/api/admin/providers/seo-rank-tracking`, which returns a redacted view
 * (`hasCredentials`) and never the stored credential itself.
 */

import { useCallback, useEffect, useState } from 'react';
import { LineChart, CheckCircle2, XCircle, Loader2, ShieldCheck, Wallet } from 'lucide-react';
...
import {
  adminGetRankTrackingProvider, adminSaveRankTrackingProvider, adminDeleteRankTrackingProvider, adminTestRankTrackingProvider,
  type AdminRankTrackingProviderInfo, type AdminRankTrackingTestResult,
} from '@/lib/api';

export function AdminRankTrackingProviderCard() {
  const { toast } = useToast();
  const [info, setInfo] = useState<AdminRankTrackingProviderInfo | null>(null);
  const [loading, setLoading] = useState(true);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [saving, setSaving] = useState(false);
  const [testing, setTesting] = useState(false);
  const [testResult, setTestResult] = useState<AdminRankTrackingTestResult | null>(null);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setInfo(await adminGetRankTrackingProvider());
    } catch {
      setInfo(null);
    } finally {
      setLoading(false);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const next = await adminSaveRankTrackingProvider({
      ...
      setPassword('');
      setDialogOpen(false);
      toast({ title: 'Rank tracking provider saved' });
    } catch (err) {
      toast({
        title: 'Could not save rank tracking provider',
  ...
  const handleTest = async () => {
    ...
    setTestResult(null);
    try {
      setTestResult(await adminTestRankTrackingProvider());
    } catch (err) {
      setTestResult({
        success: false,
  ...
  const handleDisable = async () => {
    try {
      setInfo(await adminDeleteRankTrackingProvider());
      setTestResult(null);
      toast({ title: 'Rank tracking provider removed' });
    } catch (err) {
      toast({
        title: 'Could not remove rank tracking provider',
        description: err instanceof Error ? err.message : 'Unknown error',
        variant: 'destructive',
      });
    }
  ...
  return (
    <Card className="bg-card border-border" data-testid="admin-rank-tracking-provider-card">
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between gap-2">
          <CardTitle className="text-sm flex items-center gap-2 text-foreground">
            <LineChart className="h-4 w-4 text-primary" />
            SEO Rank Tracking Provider
          </CardTitle>
          ...
          <>
            <div className="space-y-1.5 text-xs">
              ...
              <div className="flex items-center justify-between">
                ...
                {info?.hasCredentials ? (
                  <span className="flex items-center gap-1 text-emerald-500" data-testid="rank-tracking-credential-saved">
            ...
            {testResult && (
              <div
                ...
                data-testid="rank-tracking-test-result"
              >
                {testResult.success ? (
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-sm">
              <LineChart className="h-4 w-4 text-primary" /> Configure Rank Tracking Provider
            </DialogTitle>
          ...
          <div className="space-y-3">
            <div className="space-y-1.5">
              <Label htmlFor="rank-tracking-login" className="text-xs">Login</Label>
              <Input
                id="rank-tracking-login"
                autoComplete="off"
                value={login}
            ...
            <div className="space-y-1.5">
              <Label htmlFor="rank-tracking-password" className="text-xs">Password</Label>
              <Input
                id="rank-tracking-password"
                type="password"
                autoComplete="off"
                value={password}
            ...
            <div className="flex items-center justify-between rounded-md border border-border px-3 py-2">
              <Label htmlFor="rank-tracking-enabled" className="text-xs">Enabled</Label>
              <Switch id="rank-tracking-enabled" checked={enabled} onCheckedChange={setEnabled} />
            </div>
          </div>
