...
function adminAuthHeaders(): Record<string, string> {
  // Uses the user's actual JWT session token for admin auth
  const anonKey = import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY;
  return anonKey ? { 'Authorization': `Bearer ${anonKey}` } : {};
}
...
export async function adminListUsers(page = 1, perPage = 100) {
  ...
  return request<{ users: any[] }>('/api/admin/users/list', {
    method: 'POST', headers, body: JSON.stringify({ page, perPage }),
  });
}

export async function adminCreateUser(data: { email: string; password: string; fullName?: string; emailConfirm?: boolean }) {
  const headers = await getAdminHeaders();
  return request<{ success: boolean; user: { id: string; email: string } }>('/api/admin/users/create', {
...
export async function adminResetPassword(email: string) {
  const headers = await getAdminHeaders();
  return request<{ success: boolean; link?: string }>('/api/admin/users/reset-password', {
...
export async function adminUpdatePassword(userId: string, password: string) {
  const headers = await getAdminHeaders();
  return request<{ success: boolean }>('/api/admin/users/update-password', {
    method: 'POST', headers, body: JSON.stringify({ userId, password }),
  });
}

export async function adminToggleDisable(userId: string, disable: boolean) {
  const headers = await getAdminHeaders();
  return request<{ success: boolean }>('/api/admin/users/toggle-disable', {
    method: 'POST', headers, body: JSON.stringify({ userId, disable }),
  });
}

export async function adminConfirmEmail(userId: string) {
  const headers = await getAdminHeaders();
  return request<{ success: boolean }>('/api/admin/users/confirm-email', {
...
export async function adminDeleteUser(userId: string) {
  const headers = await getAdminHeaders();
  return request<{ success: boolean }>('/api/admin/users/delete', {
    method: 'POST', headers, body: JSON.stringify({ userId }),
  });
}

export async function adminAssignRole(userId: string, role: string) {
  const headers = await getAdminHeaders();
  return request<{ success: boolean }>('/api/admin/roles/assign', {
    method: 'POST', headers, body: JSON.stringify({ userId, role }),
  });
}

export async function adminRemoveRole(data: { roleId?: string; userId?: string; role?: string }) {
  const headers = await getAdminHeaders();
  return request<{ success: boolean }>('/api/admin/roles/remove', {
    method: 'POST', headers, body: JSON.stringify(data),
  });
}
...
export async function adminListRoles() {
  const headers = await getAdminHeaders();
  return request<{ roles: any[] }>('/api/admin/roles/list', { headers });
}
