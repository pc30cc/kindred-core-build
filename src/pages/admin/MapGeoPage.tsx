...
export default function AdminMapGeoPage() {
  ...
  const saveMu = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast({
        title: t('admin.mapGeo.toasts.saved' as any),
        description: data.changed
          ? t('admin.mapGeo.toasts.savedWithChanges' as any)
          : t('admin.mapGeo.toasts.savedNoChanges' as any),
      });
    },
    onError: (err: any) => toast({
      title: t('admin.mapGeo.toasts.saveFailed' as any),
  ...
  const testMu = useMutation({
    ...
    onError: (err: any) => toast({
      title: t('admin.mapGeo.toasts.testFailed' as any),
      description: err.message, variant: 'destructive',
    }),
  });
  ...
  const warmMu = useMutation({
    ...
    onSuccess: (r) => {
      setWarmResult(r);
      toast({
        title: t('admin.mapGeo.toasts.warmDone' as any),
        description: `${r.enriched} enriched · ${r.skipped_no_raw_ip} skipped (no raw IP)`,
      });
    },
    onError: (err: any) => toast({
      title: t('admin.mapGeo.toasts.warmFailed' as any),
      description: err.message, variant: 'destructive',
    }),
  });
  ...
  if (draft.allow_centroid_fallback) overviewIssues.push(t('admin.mapGeo.overview.warnCentroid' as any));
  if (!draft.store_raw_ip) overviewIssues.push(t('admin.mapGeo.overview.warnRawIp' as any));
  if (!status?.usable && draft.maxmind_local.enabled) overviewIssues.push(t('admin.mapGeo.overview.warnMmdb' as any));
  if (draft.min_accuracy_for_map === 'city') overviewIssues.push(t('admin.mapGeo.overview.warnStrict' as any));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <Link to="/admin/providers" className="hover:text-foreground inline-flex items-center gap-1">
              <ArrowLeft className="h-3 w-3" /> {t('admin.nav.providers' as any)}
            </Link>
            <span>/</span>
            <span>{t('admin.mapGeo.title' as any)}</span>
          </div>
          <h1 className="text-2xl font-bold text-foreground mt-1">{t('admin.mapGeo.title' as any)}</h1>
          <p className="text-muted-foreground text-sm">{t('admin.mapGeo.subtitle' as any)}</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" disabled={!dirty || saveMu.isPending}
            ...
            {saveMu.isPending ? t('common.loading') : t('admin.mapGeo.actions.save' as any)}
          </Button>
        </div>
      </div>
      ...
      <SectionCard
        icon={Activity}
        title={t('admin.mapGeo.overview.title' as any)}
        description={t('admin.mapGeo.overview.subtitle' as any)}
      >
        <div className="grid gap-3 grid-cols-2 md:grid-cols-4">
          <div className="p-3 rounded-md border border-border bg-muted/20">
            <p className="text-[10px] uppercase text-muted-foreground tracking-wide">{t('admin.mapGeo.overview.activeProvider' as any)}</p>
            <p className="text-sm font-semibold text-foreground mt-1">{draft.default_provider}</p>
          </div>
          <div className="p-3 rounded-md border border-border bg-muted/20">
            <p className="text-[10px] uppercase text-muted-foreground tracking-wide">{t('admin.mapGeo.overview.precision' as any)}</p>
            <p className="text-sm font-semibold text-foreground mt-1 capitalize">{draft.preferred_precision}</p>
          </div>
          <div className="p-3 rounded-md border border-border bg-muted/20">
            <p className="text-[10px] uppercase text-muted-foreground tracking-wide">{t('admin.mapGeo.overview.minAccuracy' as any)}</p>
            <p className="text-sm font-semibold text-foreground mt-1 capitalize">{draft.min_accuracy_for_map}</p>
          </div>
          <div className="p-3 rounded-md border border-border bg-muted/20">
            <p className="text-[10px] uppercase text-muted-foreground tracking-wide">{t('admin.mapGeo.overview.mmdb' as any)}</p>
            <div className="mt-1">
              {status
                ? <StatusBadge ok={!!status.usable} warning={!status.enabled}
                    label={status.usable ? t('admin.mapGeo.status.healthy' as any) : (status.enabled ? t('admin.mapGeo.status.error' as any) : t('admin.mapGeo.status.disabled' as any))} />
      ...
      {/* ─── 2. Geo Core ─── */}
      <SectionCard icon={MapPin} title={t('admin.mapGeo.core.title' as any)}
        description={t('admin.mapGeo.core.subtitle' as any)}>
        <FieldRow label={t('admin.mapGeo.core.enabled' as any)} hint={t('admin.mapGeo.core.enabledHint' as any)}>
          <Switch checked={draft.enabled} onCheckedChange={(v) => setDraft({ ...draft, enabled: v })} />
        </FieldRow>
        <FieldRow label={t('admin.mapGeo.core.defaultProvider' as any)}>
          <Input value={draft.default_provider}
            onChange={(e) => setDraft({ ...draft, default_provider: e.target.value })} />
        </FieldRow>
        <FieldRow label={t('admin.mapGeo.core.precision' as any)} hint={t('admin.mapGeo.core.precisionHint' as any)}>
          <Select value={draft.preferred_precision}
            onValueChange={(v) => setDraft({ ...draft, preferred_precision: v as any })}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="country">{t('admin.mapGeo.precision.country' as any)}</SelectItem>
              <SelectItem value="region">{t('admin.mapGeo.precision.region' as any)}</SelectItem>
              <SelectItem value="city">{t('admin.mapGeo.precision.city' as any)}</SelectItem>
            </SelectContent>
          </Select>
        </FieldRow>
        <FieldRow label={t('admin.mapGeo.core.allowCentroid' as any)} hint={t('admin.mapGeo.core.allowCentroidHint' as any)}>
          <Switch checked={draft.allow_centroid_fallback}
            onCheckedChange={(v) => setDraft({ ...draft, allow_centroid_fallback: v })} />
        </FieldRow>
        <FieldRow label={t('admin.mapGeo.core.minAccuracy' as any)} hint={t('admin.mapGeo.core.minAccuracyHint' as any)}>
        ...
        <FieldRow label={t('admin.mapGeo.core.storeRawIp' as any)} hint={t('admin.mapGeo.core.storeRawIpHint' as any)}>
          <Switch checked={draft.store_raw_ip}
            onCheckedChange={(v) => setDraft({ ...draft, store_raw_ip: v })} />
        </FieldRow>
        <FieldRow label={t('admin.mapGeo.core.retentionDays' as any)}>
        ...
        <FieldRow label={t('admin.mapGeo.core.autoEnrich' as any)}>
        ...
        {!draft.store_raw_ip && (
          <div className="flex items-start gap-2 p-2 rounded-md border border-border bg-muted/20 text-[11px] text-muted-foreground">
            <Info className="h-3.5 w-3.5 mt-0.5 shrink-0" /> {t('admin.mapGeo.core.privacyNote' as any)}
          </div>
        )}
      </SectionCard>

      {/* ─── 3. MaxMind Local ─── */}
      <SectionCard icon={Database} title={t('admin.mapGeo.mmdb.title' as any)}
        description={t('admin.mapGeo.mmdb.subtitle' as any)}>
        <FieldRow label={t('admin.mapGeo.mmdb.enabled' as any)}>
          <Switch checked={draft.maxmind_local.enabled}
            onCheckedChange={(v) => setDraft({
              ...draft, maxmind_local: { ...draft.maxmind_local, enabled: v },
            })} />
        </FieldRow>
        <FieldRow label={t('admin.mapGeo.mmdb.dbPath' as any)} hint={t('admin.mapGeo.mmdb.dbPathHint' as any)}>
          <Input value={draft.maxmind_local.db_path}
            onChange={(e) => setDraft({
              ...draft, maxmind_local: { ...draft.maxmind_local, db_path: e.target.value },
            })} />
        </FieldRow>
        <FieldRow label={t('admin.mapGeo.mmdb.autoReload' as any)}>
          <Switch checked={draft.maxmind_local.auto_reload}
            onCheckedChange={(v) => setDraft({
              ...draft, maxmind_local: { ...draft.maxmind_local, auto_reload: v },
            })} />
        </FieldRow>
        <FieldRow label={t('admin.mapGeo.mmdb.cacheTtl' as any)}>
        ...
        <div className="border-t border-border pt-3">
          <div className="flex items-center justify-between mb-2">
            <h4 className="text-xs font-semibold text-foreground">{t('admin.mapGeo.mmdb.healthTitle' as any)}</h4>
            <Button variant="outline" size="sm" onClick={() => statusQ.refetch()} disabled={statusQ.isFetching}>
              ...
              {t('admin.mapGeo.actions.refresh' as any)}
            </Button>
          </div>
          {status ? (
            <div className="grid gap-2 md:grid-cols-2 text-[11px]">
              <div className="p-2 rounded-md border border-border bg-muted/20 flex items-center justify-between">
                <span className="text-muted-foreground">{t('admin.mapGeo.mmdb.field.configured' as any)}</span>
                <StatusBadge ok={status.configured} label={status.configured ? t('admin.mapGeo.status.yes' as any) : t('admin.mapGeo.status.no' as any)} />
              </div>
              <div className="p-2 rounded-md border border-border bg-muted/20 flex items-center justify-between">
                <span className="text-muted-foreground">{t('admin.mapGeo.mmdb.field.fileExists' as any)}</span>
                <StatusBadge ok={status.file_exists} label={status.file_exists ? t('admin.mapGeo.status.yes' as any) : t('admin.mapGeo.status.no' as any)} />
              </div>
              <div className="p-2 rounded-md border border-border bg-muted/20 flex items-center justify-between">
                <span className="text-muted-foreground">{t('admin.mapGeo.mmdb.field.usable' as any)}</span>
                <StatusBadge ok={status.usable} label={status.usable ? t('admin.mapGeo.status.yes' as any) : t('admin.mapGeo.status.no' as any)} />
              </div>
              <div className="p-2 rounded-md border border-border bg-muted/20 flex items-center justify-between">
                <span className="text-muted-foreground">{t('admin.mapGeo.mmdb.field.size' as any)}</span>
              ...
              <div className="md:col-span-2 p-2 rounded-md border border-border bg-muted/20">
                <span className="text-muted-foreground">{t('admin.mapGeo.mmdb.field.path' as any)}: </span>
              ...
              {status.mtime && (
                <div className="md:col-span-2 text-muted-foreground">
                  {t('admin.mapGeo.mmdb.field.mtime' as any)}: {new Date(status.mtime).toLocaleString()}
                </div>
              )}
            </div>
          ) : statusQ.isLoading ? (
        ...
        <div className="text-[11px] text-muted-foreground border-t border-border pt-2 leading-relaxed">
          ...
          {t('admin.mapGeo.mmdb.deployHint' as any)}
        </div>
      </SectionCard>

      {/* ─── 4. Map Behavior ─── */}
      <SectionCard icon={MapIcon} title={t('admin.mapGeo.map.title' as any)}
        description={t('admin.mapGeo.map.subtitle' as any)}>
        <FieldRow label={t('admin.mapGeo.map.showOnlyValid' as any)}>
          <Switch checked={draft.map.show_only_valid_coords}
            onCheckedChange={(v) => setDraft({ ...draft, map: { ...draft.map, show_only_valid_coords: v } })} />
        </FieldRow>
        <FieldRow label={t('admin.mapGeo.map.ignoreFallback' as any)} hint={t('admin.mapGeo.map.ignoreFallbackHint' as any)}>
        ...
        <FieldRow label={t('admin.mapGeo.map.includeLabels' as any)}>
          <Switch checked={draft.map.include_geo_labels}
            onCheckedChange={(v) => setDraft({ ...draft, map: { ...draft.map, include_geo_labels: v } })} />
        </FieldRow>
        <FieldRow label={t('admin.mapGeo.map.debugMode' as any)} hint={t('admin.mapGeo.map.debugModeHint' as any)}>
          <Switch checked={draft.map.debug_mode}
            onCheckedChange={(v) => setDraft({ ...draft, map: { ...draft.map, debug_mode: v } })} />
        </FieldRow>
        <FieldRow label={t('admin.mapGeo.map.centerMode' as any)}>
          <Select value={draft.map.default_center_mode}
            onValueChange={(v) => setDraft({ ...draft, map: { ...draft.map, default_center_mode: v as any } })}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="auto">{t('admin.mapGeo.map.auto' as any)}</SelectItem>
              <SelectItem value="manual">{t('admin.mapGeo.map.manual' as any)}</SelectItem>
            </SelectContent>
          </Select>
        </FieldRow>
        <FieldRow label={t('admin.mapGeo.map.defaultLat' as any)}>
        ...
        <FieldRow label={t('admin.mapGeo.map.defaultLng' as any)}>
        ...
        <FieldRow label={t('admin.mapGeo.map.defaultZoom' as any)}>
      ...
      {/* ─── 5. Geo Jobs ─── */}
      <SectionCard icon={RefreshCw} title={t('admin.mapGeo.jobs.title' as any)}
        description={t('admin.mapGeo.jobs.subtitle' as any)}>
        <FieldRow label={t('admin.mapGeo.jobs.lookback' as any)}>
        ...
        <FieldRow label={t('admin.mapGeo.jobs.limit' as any)}>
        ...
        <FieldRow label={t('admin.mapGeo.jobs.force' as any)} hint={t('admin.mapGeo.jobs.forceHint' as any)}>
        ...
        <div className="flex items-center gap-2 pt-2 border-t border-border">
          <Button onClick={() => warmMu.mutate({
            ...
            {warmMu.isPending ? t('common.loading') : t('admin.mapGeo.jobs.run' as any)}
          </Button>
          {warmResult && (
            <span className="text-[11px] text-muted-foreground">
              {t('admin.mapGeo.jobs.lastRun' as any)}
            </span>
          )}
        </div>
        {warmResult && (
      ...
      {/* ─── 6. Diagnostics ─── */}
      <SectionCard icon={Bug} title={t('admin.mapGeo.diag.title' as any)}
        description={t('admin.mapGeo.diag.subtitle' as any)}>
        <div className="flex items-end gap-2">
          <div className="flex-1">
            <Label className="text-xs">{t('admin.mapGeo.diag.ip' as any)}</Label>
          ...
          <Button onClick={() => testMu.mutate(testIp.trim())}
            ...
            <PlayCircle className="h-3.5 w-3.5 me-1" />
            {testMu.isPending ? t('common.loading') : t('admin.mapGeo.diag.run' as any)}
          </Button>
        </div>
        {testResult && (
          <div className="space-y-2">
            <div className="grid gap-2 md:grid-cols-2 text-[11px]">
              <div className="p-2 rounded-md border border-border bg-muted/20"><span className="text-muted-foreground">{t('admin.mapGeo.diag.field.provider' as any)}: </span><span className="font-mono text-foreground">{testResult.result.source_provider ?? '—'}</span></div>
              <div className="p-2 rounded-md border border-border bg-muted/20"><span className="text-muted-foreground">{t('admin.mapGeo.diag.field.source' as any)}: </span><span className="font-mono text-foreground">{testResult.result.source}</span></div>
              <div className="p-2 rounded-md border border-border bg-muted/20"><span className="text-muted-foreground">{t('admin.mapGeo.diag.field.country' as any)}: </span><span className="text-foreground">{testResult.result.country ?? '—'} ({testResult.result.country_code ?? '—'})</span></div>
              <div className="p-2 rounded-md border border-border bg-muted/20"><span className="text-muted-foreground">{t('admin.mapGeo.diag.field.region' as any)}: </span><span className="text-foreground">{testResult.result.region ?? '—'}</span></div>
              <div className="p-2 rounded-md border border-border bg-muted/20"><span className="text-muted-foreground">{t('admin.mapGeo.diag.field.city' as any)}: </span><span className="text-foreground">{testResult.result.city ?? '—'}</span></div>
              <div className="p-2 rounded-md border border-border bg-muted/20"><span className="text-muted-foreground">{t('admin.mapGeo.diag.field.coords' as any)}: </span><span className="font-mono text-foreground">{testResult.result.latitude?.toFixed(4) ?? '—'}, {testResult.result.longitude?.toFixed(4) ?? '—'}</span></div>
              <div className="p-2 rounded-md border border-border bg-muted/20"><span className="text-muted-foreground">{t('admin.mapGeo.diag.field.timezone' as any)}: </span><span className="text-foreground">{testResult.result.timezone ?? '—'}</span></div>
              <div className="p-2 rounded-md border border-border bg-muted/20"><span className="text-muted-foreground">{t('admin.mapGeo.diag.field.accuracy' as any)}: </span><span className="font-mono capitalize text-foreground">{testResult.result.accuracy_level ?? '—'}</span></div>
              <div className="p-2 rounded-md border border-border bg-muted/20 md:col-span-2 flex items-center justify-between">
                <span className="text-muted-foreground">{t('admin.mapGeo.diag.field.fallback' as any)}</span>
                <StatusBadge ok={!testResult.result.is_fallback}
                  warning={testResult.result.is_fallback}
                  label={testResult.result.is_fallback ? t('admin.mapGeo.status.fallback' as any) : t('admin.mapGeo.status.precise' as any)} />
              </div>
            </div>
