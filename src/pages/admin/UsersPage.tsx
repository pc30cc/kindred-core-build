...
function UserRolesCell({ userId }: { userId: string }) {
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      {currentRoles.length === 0 && <span className="text-muted-foreground text-xs">No roles</span>}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm text-muted-foreground">{count ?? 0} total users</span>
      </div>
            ...
            <TableBody>
              ...
              {profiles?.map(p => (
                <TableRow key={p.id} className="border-border hover:bg-muted/50">
                  <TableCell className="text-foreground font-mono text-sm">{p.email}</TableCell>
                  <TableCell className="text-foreground/80">{p.full_name || '—'}</TableCell>
