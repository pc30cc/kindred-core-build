...
vi.mock('../../../server/services/widget/anonymousContact.js', () => ({ ensureVisitorContact: async () => { contacts += 1; return null; } }));
...
describe('writes only when something changed', () => {
  ...
  it('the same shopper again: no link write, no contact write (only the replay nonce)', async () => {
    await bind();
    fake.reset();
    const r = await bind();
  ...
  it('another shopper in the same browser: the link switches and a cutoff is recorded', async () => {
    await bind();
    const r = await bind(assertion({ external_customer_id: '102', session_ref: 'ref-c', customer_group_id: '2' }));
    expect(r.outcome).toBe('switched');
    expect(fake.db.commerce_customer_links).toHaveLength(1);
    expect(fake.db.commerce_customer_links[0]).toMatchObject({ external_customer_id: '102', customer_group_id: '2' });
    expect(fake.db.commerce_customer_links[0].private_cutoff_at).toBeTruthy();
