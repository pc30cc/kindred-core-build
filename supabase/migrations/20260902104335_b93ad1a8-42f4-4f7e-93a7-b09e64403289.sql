ALTER TABLE public.workspace_seat_entitlement_mode
  ADD COLUMN IF NOT EXISTS seat_limit integer;

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint
                 WHERE conrelid = 'public.workspace_seat_entitlement_mode'::regclass
                   AND conname = 'workspace_seat_entitlement_mode_seat_limit_check') THEN
    ALTER TABLE public.workspace_seat_entitlement_mode
      ADD CONSTRAINT workspace_seat_entitlement_mode_seat_limit_check
...
-- HOSTED CHAIN BODY: the hosted schema owns workspace_subscriptions and
-- billing_plans, so the authority is read with static SQL (no to_regclass,
-- no dynamic SQL). Missing/unreadable entitlement fails closed.
...
INSERT INTO public.workspace_seat_entitlement_mode (id, mode, source, seat_limit)
VALUES (true, 'plan_authoritative', 'hosted_bootstrap', NULL)
...
BEGIN
  ...
  PERFORM public.wi_audit(_workspace_id, _actor_id, 'invitation.archived', _invitation_id, '{}'::jsonb);
