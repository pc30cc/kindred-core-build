-- Hosted mirror of database/migrations/089_workspace_invitations_v51_otp_outbox.sql
...
REVOKE ALL ON FUNCTION public.wi_execute_idempotent(text,text,text,text,uuid,uuid,uuid,jsonb,integer) FROM PUBLIC;
DO $acl2$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'anon') THEN
    REVOKE ALL ON FUNCTION public.wi_execute_idempotent(text,text,text,text,uuid,uuid,uuid,jsonb,integer) FROM anon;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'authenticated') THEN
    REVOKE ALL ON FUNCTION public.wi_execute_idempotent(text,text,text,text,uuid,uuid,uuid,jsonb,integer) FROM authenticated;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'service_role') THEN
    GRANT EXECUTE ON FUNCTION public.wi_execute_idempotent(text,text,text,text,uuid,uuid,uuid,jsonb,integer) TO service_role;
  END IF;
END
$acl2$;