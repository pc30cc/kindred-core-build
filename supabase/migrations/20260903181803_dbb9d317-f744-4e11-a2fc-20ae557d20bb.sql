-- Proforma snapshot + document-number uniqueness + financial ACL convergence.
...
COMMENT ON COLUMN public.billing_payment_intents.discount_irr IS
  'Real discount applied to this proforma, in stored Rial. 0 when there is none.';
  ...
  FOR fn IN
    SELECT p.oid::regprocedure::text
     ...
     WHERE n.nspname = 'public'
       AND p.prosecdef
       AND p.proname IN ('billing_apply_subscription_payment')
  LOOP
