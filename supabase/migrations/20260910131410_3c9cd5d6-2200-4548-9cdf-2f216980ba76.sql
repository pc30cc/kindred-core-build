...
BEGIN
  ...
  IF sig IS NULL THEN RAISE EXCEPTION 'create_workspace_atomic missing'; END IF;
  IF has_function_privilege('anon', sig, 'EXECUTE') OR has_function_privilege('authenticated', sig, 'EXECUTE') THEN
    RAISE EXCEPTION 'create_workspace_atomic has unsafe client execution privilege';
  END IF;
  ...
  IF sig IS NULL THEN RAISE EXCEPTION 'provision_account_on_signup missing'; END IF;
  IF has_function_privilege('anon', sig, 'EXECUTE') OR has_function_privilege('authenticated', sig, 'EXECUTE') THEN
    RAISE EXCEPTION 'provision_account_on_signup has unsafe client execution privilege';
  END IF;
  IF NOT has_function_privilege('service_role', sig, 'EXECUTE') THEN
